...
func TestSignAndVerifyReleaseToken_Success(t *testing.T) {
	...
	assert.Equal(t, "0.5.0", parsedClaims.Version)
	assert.Equal(t, "4b825dc642cb6eb9a060e54bf8d69288fbee4904", parsedClaims.GitCommit)
	assert.Equal(t, "2026-09-11T12:00:00Z", parsedClaims.BuildDate)
...
func TestEvaluateProvenance_UnattestedCustomBuild(t *testing.T) {
	...
	// Layer 3 Defense: Unattested build NEVER converts to Apache 2.0 even if 4 years elapse!
...
func TestEvaluateProvenance_TamperedMetadata(t *testing.T) {
	...
	// Signed for v0.5.0 and commit aaa111
	claims := &version.ReleaseClaims{
		Version:   "0.5.0",
		GitCommit: "aaa111",
		BuildDate: "2026-09-11T12:00:00Z",
...
func TestEvaluateProvenance_TamperedSignature(t *testing.T) {
	...
	// Set public key to pubWrong, but signed with privSigner
...
func TestSignAndVerifyReleaseToken_ArmoredPEM(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "gitlab-fleet-governor", parsed.Product)
	assert.Equal(t, "0.6.0", parsed.Version)
...
func TestEvaluateProvenance_LegacyTwoPartToken(t *testing.T) {
	...
	legacyPayload := `{"version":"0.5.0","git_commit":"c0ffee","build_date":"2026-09-11T12:00:00Z","authority":"DIVMORA Technologies"}`
