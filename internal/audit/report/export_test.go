...
func TestExportFormats(t *testing.T) {
	...
	t.Run("JSON", func(t *testing.T) {
		...
		require.NotNil(t, parsed.LicenseAttestation)
		assert.Equal(t, "VALID_COMMERCIAL", parsed.LicenseAttestation.Status)
		assert.Equal(t, "BSL-1.1", parsed.LicenseAttestation.LicenseModel)
		assert.Equal(t, "Acme Corp", parsed.LicenseAttestation.LicensedTo)
		assert.Equal(t, 1000, parsed.LicenseAttestation.MaxProjects)
	})
	...
	t.Run("Markdown", func(t *testing.T) {
		...
		assert.Contains(t, mdContent, "## Commercial Licensing & Statutory Attestation")
		assert.Contains(t, mdContent, "BSL-1.1")
		assert.Contains(t, mdContent, "VALID_COMMERCIAL")
		assert.Contains(t, mdContent, "Acme Corp")
		assert.Contains(t, mdContent, "1000 Projects")
	...
	t.Run("HTML", func(t *testing.T) {
		...
		assert.Contains(t, htmlContent, "Commercial Licensing & Statutory Attestation")
		assert.Contains(t, htmlContent, "VALID_COMMERCIAL")
	...
	t.Run("Table", func(t *testing.T) {
		...
		assert.Contains(t, tableContent, "Licensing      : BSL-1.1 (Tier: ENTERPRISE) | Status: VALID_COMMERCIAL | Licensed To: Acme Corp")
		assert.Contains(t, tableContent, "Attestation    :")
